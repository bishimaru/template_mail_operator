...
def jmail_footprints(driver, wait):
  dbpath = 'firstdb.db'
  ...
  # # 順番
  # # データ検索
  cur.execute('SELECT name, login_id, login_passward FROM jmail')
  jmail_user_list = []
  foot_order_list = [
     "いおり", "きりこ", "つむぎ", "ハル", "りこ", "ゆうな", "ゆっこ"
  ]
  # foot_order_list = ["アスカ",
  #                    ]
  ...
  for i in range(9999):
    ...
    for user_list in jmail_user_list:
        if user_list[1] == None or user_list[1] == "":
           continue
        try:
          jmail.make_footprints(user_list[0], user_list[1], user_list[2], driver, wait)
